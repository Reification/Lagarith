...
FORCE_INLINE __m128i lagx_mm_loadl_epi64(const __m128i* a) {
	const int64_t ALIGN_STRUCT(16) data[2] = {((const int64_t*)a)[0], 0ll};
	return vreinterpretq_m128i_s64(vld1q_s64(data));
}

FORCE_INLINE __m128i lagx_mm_setr_epi8(int8_t i15, int8_t i14, int8_t i13, int8_t i12, int8_t i11,
...
FORCE_INLINE __m128i lagx_mm_alignr_epi8(__m128i a, __m128i b, uint32_t ralign) {
...
FORCE_INLINE __m128i lagx_mm_unpacklo_epi64(__m128i a, __m128i b) {
...
FORCE_INLINE __m128i lagx_mm_lddqu_si128(const __m128i* v) {
	const int32_t* pv                       = (const int32_t*)&v;
	const int32_t  ALIGN_STRUCT(16) data[4] = {pv[0], pv[1], pv[2], pv[3]};
	return vreinterpretq_m128i_s32(vld1q_s32(data));
}

FORCE_INLINE __m128i lagx_mm_shuffle_epi8(__m128i v, __m128i s) {
	const int8_t* ps = (const int8_t*)&s;
	const int8_t* pv = (const int8_t*)&v;
#define _vb(i) ((ps[i] & 0xf0) ? ((int8_t)0) : pv[ps[i]])
...
FORCE_INLINE __m128i lagx_mm_shufflelo_epi16(__m128i v, int s) {
	const int16_t* pv                        = (const int16_t*)&v;
	const int16_t  ALIGN_STRUCT(16) data[16] = {
    pv[s & 0x3], pv[(s >> 2) & 0x3], pv[(s >> 4) & 0x3], pv[(s >> 6) & 0x3]
...
FORCE_INLINE uint64_t lagx__emulu(uint32_t a, uint32_t b) {
